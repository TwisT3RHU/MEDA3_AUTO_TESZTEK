...
test('új távoli szerver hozzáadása', async ({ page }) => {
  ...
  await page.locator('td[role="listitem"]:has-text("'+ remotename +'")').click();
  await page.getByRole('cell', { name: user.name }).first().click();
  await page.getByRole('button', { name: ' Hozzáad' }).first().click();
  await page.getByRole('row', { name: remotename + " " + user.name }).getByRole('cell', { name: user.name }).click();
  ...
  await page2.locator('td[role="listitem"]:has-text("'+ servername +'")').click();
  await page2.getByRole('cell', { name: user.name }).first().click();
  await page2.getByRole('button', { name: ' Hozzáad' }).first().click();
  await page2.getByRole('row', { name: servername + " " + user.name }).getByRole('cell', { name: user.name }).click();
