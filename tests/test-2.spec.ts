...
test.describe.serial('egy felhasználót érintő tesztek', () => {
  ...
  test('felhasználó létrehozása', async ({ page }) => {
    ...
    await page.getByRole('textbox', { name: 'Jelszó mégegyszer' }).fill(testname);
    await page.getByRole('textbox', { name: 'Teljes név' }).fill('autoteszt user');
    await page.getByRole('textbox', { name: 'Teljes név' }).press('Tab');
    await page.getByRole('textbox', { name: 'Email cím' }).fill('autoteszt@example.com');
    await page.getByRole('textbox', { name: 'Email cím' }).press('Tab');
    await page.getByRole('textbox', { name: 'Bejelentkező kód' }).fill(testname);
    ...
    await page.getByRole('row', { name: 'Alapértelmezett nyelv' }).getByRole('combobox').locator('div').click();
  ...
  test('felhasználó hozzáadása egy csoporthoz', async ({ page }) => {
    ...
    await page.getByRole('row', { name: /.*geriautocsop/ }).locator('span').click(); // EGYELŐRE NEM TUDOM FUTURE-PROOFOLNI :)
    await page.getByRole('button', { name: ' Hozzáad' }).click();
    console.log(testname + " hozzáadva egy csoporthoz");
    
  });

  test('felhasználó eltávolítása egy csoportból + megerősítő ablak', async ({ page }) => {
  
    await page.getByRole('cell', { name: testname }).click();
    await page.locator('.v-splitpanel-second-container > .v-panel > .v-panel-content > .v-verticallayout > .v-expand > div > .v-grid > .v-grid-tablewrapper > table > .v-grid-body > tr > td').first().click();
  ...
  test('felhasználó adatainak módosítása', async ({ page }) => {
    ...
    await page.getByRole('textbox', { name: 'Jelszó' }).press('Tab');
    await page.getByRole('textbox', { name: 'Jelszó mégegyszer' }).fill(testnamedit);
    await page.getByRole('textbox', { name: 'Teljes név' }).fill(testnamedit);
    await page.getByRole('textbox', { name: 'Teljes név' }).press('Tab');
    await page.getByRole('textbox', { name: 'Email cím' }).fill('autoteszt_edited@example.com');
    await page.getByRole('textbox', { name: 'Email cím' }).press('Tab');
    await page.getByRole('textbox', { name: 'Bejelentkező kód' }).fill(testnamedit);
    //await page.getByRole('row', { name: 'B. kód érvényesség' }).locator('button').click();
    //await page.getByRole('button', { name: '»' }).click();
    //await page.getByRole('row', { name: '23 24 25 26 27 28 29' }).getByText('29').click();
    //await page.getByRole('row', { name: 'Partner' }).getByRole('combobox').locator('div').click();
...
test.describe.serial(misc.bulkcount + ' felhasználót érintő tesztek', () => {
  ...
  test(misc.bulkcount + ' felhasználó létrehozása', async ({ page }) => {
    ...
    for (let index = 1; index < misc.bulkcount + 1; index++) {
      ...
      await page.getByRole('textbox', { name: 'Jelszó' }).press('Tab');
      await page.getByRole('textbox', { name: 'Jelszó mégegyszer' }).fill(randomname2);
      await page.getByRole('textbox', { name: 'Teljes név' }).fill('autoteszt user ' + index);
      await page.getByRole('textbox', { name: 'Teljes név' }).press('Tab');
      await page.getByRole('textbox', { name: 'Email cím' }).fill('autoteszt@' + index + 'example.com');
      await page.getByRole('textbox', { name: 'Email cím' }).press('Tab');
      await page.getByRole('textbox', { name: 'Bejelentkező kód' }).fill(randomname2);
      //await page.getByRole('row', { name: 'B. kód érvényesség' }).locator('button').click();
      //await page.getByRole('button', { name: '»' }).click();
      //await page.getByRole('row', { name: '23 24 25 26 27 28 29' }).getByText('29').click();
      await page.getByRole('row', { name: 'Partner' }).getByRole('combobox').locator('div').click();
  ...
  test(misc.bulkcount + ' felhasználó adatainak módosítása', async ({ page }) => {
    ...
    for (let index = 1; index < misc.bulkcount + 1; index++) {
      ...
      await page.getByRole('textbox', { name: 'Jelszó' }).fill(randomname2edit);
      await page.getByRole('textbox', { name: 'Jelszó' }).press('Tab');
      await page.getByRole('textbox', { name: 'Jelszó mégegyszer' }).fill(randomname2edit);
      await page.getByRole('textbox', { name: 'Teljes név' }).fill('autoteszt user_edited' + index);
      await page.getByRole('textbox', { name: 'Teljes név' }).press('Tab');
      await page.getByRole('textbox', { name: 'Email cím' }).fill('autoteszt_edited@' + index + 'example.com');
      await page.getByRole('textbox', { name: 'Email cím' }).press('Tab');
      await page.getByRole('textbox', { name: 'Bejelentkező kód' }).fill(randomname2edit);
      //await page.getByRole('row', { name: 'B. kód érvényesség' }).locator('button').click();
      //await page.getByRole('button', { name: '»' }).click();
      //await page.getByRole('row', { name: '23 24 25 26 27 28 29' }).getByText('29').click();
      //await page.getByRole('row', { name: 'Partner' }).getByRole('combobox').locator('div').click();
  ...
  test(misc.bulkcount + ' felhasználó törlése', async ({ page }) => {
    ...
    for (let index = 1; index < misc.bulkcount + 1; index++) {
      const randomname2edit = testname + '_' + index; + '_edited';
